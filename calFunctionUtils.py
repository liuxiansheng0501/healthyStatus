...
def grPitch1MotorOpetationTime(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time,
                               his):  # 桨叶1电机工作时间
    # TODO-TESTED
...
def grPitch1BackupPowerCapacity(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time,
                                his):  # 桨叶1后备电源容量
    # TODO-TESTED
...
def grPitch1FollowingDifference(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time,
                                his):  # 桨叶1跟随偏差
...
def grPitch2MotorOpetationTime(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time,
                               his):  # 桨叶2电机工作时间
    # TODO-TESTED
    res = descend(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time, his)
    if res is None:  # 如果返回值为空
...
def grPitch2BackupPowerCapacity(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time,
                                his):  # 桨叶2后备电源容量
    # TODO-TESTED
    res = ascend(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time, his)
    if res is None:  # 如果返回值为空
...
def grPitch2FollowingDifference(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time,
                                his):  # 桨叶2跟随偏差
    # TODO-TESTED
    return 1
    # res=descend(index, fromValue, toValue, wtgs_id,from_time_now,current_time,to_time,his)
...
def grPitch3MotorOpetationTime(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time,
                               his):  # 桨叶3电机工作时间
    # TODO-TESTED
    res = descend(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time, his)
    if res is None:  # 如果返回值为空
...
def grPitch3BackupPowerCapacity(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time,
                                his):  # 桨叶3后备电源容量
    # TODO-TESTED
    res = ascend(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time, his)
    if res is None:  # 如果返回值为空
...
def grPitch3FollowingDifference(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time,
                                his):  # 桨叶3跟随偏差
    # TODO-TESTED
    return 1
    # res=descend(index, fromValue, toValue, wtgs_id,from_time_now,current_time,to_time,his)
...
def grGearboxMainBearingTemperature(attribute, wtgs_id, current_time, his):  # 齿轮箱主轴承温度
    ...
    if queryRunMode(wtgs_id, current_time) != 14:
        ...
        for delta in range(-5, 5):
            ...
            grGearboxMainbearingTemperature1HoursAgo = getFloatInterpoValuesFromGolden2(
                'grgearboxmainbearingtemperature', wtgs_id, hisOrFurTime(base_time_loop, -5, -1, 0),
                hisOrFurTime(base_time_loop, 0, -1, 0), his)
            ...
            grWindSpeedValue_10MIN = getFloatInterpoValuesFromGolden2('grwindspeed', wtgs_id,
                                                                      hisOrFurTime(base_time_loop, -1800, 0, 0),
                                                                      hisOrFurTime(base_time_loop, 0, 0, 0), his)
            # 输入变量-1小时功率平均值：grgridactivepower
            grGridActivepower_1hour = getFloatInterpoValuesFromGolden2('grgridactivepower', wtgs_id,
                                                                       hisOrFurTime(base_time_loop, -3600, 0, 0),
                                                                       hisOrFurTime(base_time_loop, 0, 0, 0), his)
            # 输出变量-齿轮箱主轴承温度：grgridactivepower
            grGearboxMainbearingTemperature_now = getFloatInterpoValuesFromGolden2('grgearboxmainbearingtemperature',
                                                                                   wtgs_id,
                                                                                   hisOrFurTime(base_time_loop, -5, 0,
                                                                                                0),
                                                                                   hisOrFurTime(base_time_loop, 0, 0,
                                                                                                0), his)
            actual_value_list.append(grGearboxMainbearingTemperature_now)
            # 输入变量向量
            argv_dict['input_vector'] = [grOutdoorTemperatureValue, grAirDensityValue,
                                         grGearboxMainbearingTemperature1HoursAgo, grGridActivePowerValue_30MIN,
                                         grGridActivePowerValue_10MIN, runtime, halttime, grGeneratorSpeed1Value_10MIN,
                                         grWindSpeedValue_10MIN, grGridActivepower_1hour]
            if None in argv_dict['input_vector'] or grGearboxMainbearingTemperature_now is None or int(
        ...
        if len(predict_value_list) > 0:
            healthy_score = ANNLinearDescend(float(attribute['healthylevel0']), float(attribute['healthylevel100']),
                                             meanData(actual_value_list) - meanData(predict_value_list))
        else:
            healthy_score = query_last_state(wtgs_id, current_time, 'grGearboxMainBearingTemperature',
                                             'healthy_state_index')
...
def grGearboxDETemperature(attribute, wtgs_id, current_time, his):  # 齿轮箱轮毂侧轴承温度
    ...
    if queryRunMode(wtgs_id, current_time) != 14:
        ...
        for delta in range(-5, 5):
            ...
            grGearboxoilPressureA1Value = getFloatInterpoValuesFromGolden2('grgearboxoilpressurea1', wtgs_id,
                                                                           hisOrFurTime(base_time_loop, -5, 0, 0),
                                                                           hisOrFurTime(base_time_loop, 0, 0, 0), his)
            # 输入变量-A2口压力：grgearboxoilpressurea2
            grGearboxoilPressureA2Value = getFloatInterpoValuesFromGolden2('grgearboxoilpressurea2', wtgs_id,
                                                                           hisOrFurTime(base_time_loop, -5, 0, 0),
                                                                           hisOrFurTime(base_time_loop, 0, 0, 0), his)
            # 输入变量-室外温度：groutdoortemperature
            grOutdoorTemperatureValue = getFloatInterpoValuesFromGolden2('groutdoortemperature', wtgs_id,
            ...
            grGearboxOilLevel_10MIN = getFloatInterpoValuesFromGolden2('grgearboxoillevel', wtgs_id,
                                                                       hisOrFurTime(base_time_loop, -600, 0, 0),
                                                                       hisOrFurTime(base_time_loop, 0, 0, 0), his)
            # 实际输出变量-齿轮箱前轴承温度：grgearboxhubsidebearingtemperature
            grGearboxHubsideBearingTemperature = getFloatInterpoValuesFromGolden2('grgearboxhubsidebearingtemperature',
                                                                                  wtgs_id,
                                                                                  hisOrFurTime(base_time_loop, -5, 0,
                                                                                               0),
            ...
            # 输入变量向量
            argv_dict['input_vector'] = [grGearboxoilTempertureValue, grGearboxoilPressureA1Value,
                                         grGearboxoilPressureA2Value, grOutdoorTemperatureValue, grAirDensityValue,
                                         grGridActivePowerValue_30MIN, grGridActivePowerValue_10MIN, runtime, halttime,
                                         grGeneratorSpeed1Value_10MIN, grWindSpeedValue_10MIN, grGearboxOilLevel_10MIN]
            if None in argv_dict['input_vector'] or grGearboxHubsideBearingTemperature is None or int(
                    grRunMode) != 14:  # 空或非并网的时候不运算
                pass
            else:
                ANN = BP(argv_dict)  # 采用神经网络
...
def grGearboxNDETemperature(attribute, wtgs_id, current_time, his):  # 齿轮箱发电机侧轴承温度
    ...
    if queryRunMode(wtgs_id, current_time) != 14:
        ...
        for delta in range(-5, 5):
            ...
            grGearboxoilPressureA3Value = getFloatInterpoValuesFromGolden2('grgearboxoilpressurea3', wtgs_id,
                                                                           hisOrFurTime(base_time_loop, -5, 0, 0),
                                                                           hisOrFurTime(base_time_loop, 0, 0, 0), his)
            # 输入变量-A4口压力：grgearboxoilpressurea4
            grGearboxoilPressureA4Value = getFloatInterpoValuesFromGolden2('grgearboxoilpressurea4', wtgs_id,
                                                                           hisOrFurTime(base_time_loop, -5, 0, 0),
                                                                           hisOrFurTime(base_time_loop, 0, 0, 0), his)
            # 输入变量-室外温度：groutdoortemperature
            grOutdoorTemperatureValue = getFloatInterpoValuesFromGolden2('groutdoortemperature', wtgs_id,
            ...
            grWindSpeedValue_10MIN = getFloatInterpoValuesFromGolden2('grwindspeed', wtgs_id,
                                                                      hisOrFurTime(base_time_loop, -300, 0, 0),
                                                                      hisOrFurTime(base_time_loop, 300, 0, 0), his)
            # 实际输出变量-齿轮箱后轴承温度：grgearboxgeneratorsidebearingtemperature
            grGearboxGeneratorsideBearingTemperature_Now = getFloatInterpoValuesFromGolden2(
                'grgearboxgeneratorsidebearingtemperature', wtgs_id, hisOrFurTime(base_time_loop, -5, 0, 0),
                hisOrFurTime(base_time_loop, 0, 0, 0), his)
            actual_value_list.append(grGearboxGeneratorsideBearingTemperature_Now)
            # 输入变量向量
            argv_dict['input_vector'] = [grGearboxoilTempertureValue, grGearboxoilPressureA1Value,
                                         grGearboxoilPressureA2Value,
                                         grGearboxoilPressureA3Value, grGearboxoilPressureA4Value,
                                         grOutdoorTemperatureValue,
            ...
            if None in argv_dict['input_vector'] or grGearboxGeneratorsideBearingTemperature_Now is None or int(
                    grRunMode) != 14:  # 空或非并网的时候不运算
                pass
            else:
                ANN = BP(argv_dict)  # 采用神经网络
...
def grGeneratorFloatingVoltage(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time,
                               his):  # 发电机空载电压
    # TODO-TESTED-发电机空载电压
...
def grGeneratorWindingTemperature1(attribute, wtgs_id, current_time, his):  # 发电机绕组1温度
    ...
    if queryRunMode(wtgs_id, current_time) != 14:
        ...
        for delta in range(-5, 5):
            ...
            grGeneratorWindingTemperature1 = getFloatInterpoValuesFromGolden2('grgeneratorwindingtemperature1', wtgs_id,
                                                                              hisOrFurTime(base_time_loop, -5, 0, 0),
                                                                              hisOrFurTime(base_time_loop, 0, 0, 0),
                                                                              his)
            ...
            if None in argv_dict['input_vector'] or grGeneratorWindingTemperature1 is None or int(
                    grRunMode) != 14:  # 空或非并网的时候不运算
                pass
            else:
                ANN = BP(argv_dict)  # 采用神经网络
                predict_value_list.append(ANN.output)  # 神经网络预期输出
        if len(predict_value_list) > 0:
            if str(wtgs_id) == '30002012':  # 12#机组在上限值基础上增加20%
                healthy_score = ANNLinearDescend(float(attribute['healthylevel0']) * 1.2,
                                                 float(attribute['healthylevel100']),
                                                 meanData(actual_value_list) - meanData(predict_value_list))
            else:
                healthy_score = ANNLinearDescend(float(attribute['healthylevel0']), float(attribute['healthylevel100']),
                                                 meanData(actual_value_list) - meanData(predict_value_list))
        else:
            healthy_score = query_last_state(wtgs_id, current_time, 'grGeneratorWindingTemperature1',
                                             'healthy_state_index')
        return healthy_score
...
def grGeneratorWindingTemperature2(attribute, wtgs_id, current_time, his):  # 发电机绕组2温度
    ...
    if queryRunMode(wtgs_id, current_time) != 14:
        ...
        if len(predict_value_list) > 0:
            ...
            healthy_score = query_last_state(wtgs_id, current_time, 'grGeneratorWindingTemperature2',
                                             'healthy_state_index')
        return healthy_score
    ...
    if queryRunMode(wtgs_id, current_time) != 14:
        ...
        for delta in range(-5, 5):
            ...
            argv_dict['input_vector'] = [grGearboxoilTempertureValue, grGridActivePowerValue, grOutdoorTemperatureValue,
                                         grAirDensityValue, grGridActivePowerValue_30MIN, grGridActivePowerValue_10MIN,
                                         runtime, halttime, grGeneratorSpeed1Value_10MIN, grGridActivePowerValue_1HOUR]
            if None in argv_dict['input_vector'] or grgeneratorwindingtemperature3 is None or int(
                    grRunMode) != 14:  # 空或非并网的时候不运算
                pass
            else:
                ANN = BP(argv_dict)  # 采用神经网络
        ...
        if len(predict_value_list) > 0:
            ...
            healthy_score = query_last_state(wtgs_id, current_time, 'grGeneratorWindingTemperature3',
                                             'healthy_state_index')
        return healthy_score
    ...
    if queryRunMode(wtgs_id, current_time) != 14:
        ...
        for delta in range(-5, 5):
            ...
            argv_dict['input_vector'] = [grGearboxoilTempertureValue, grGridActivePowerValue, grOutdoorTemperatureValue,
                                         grAirDensityValue, grGridActivePowerValue_30MIN, grGridActivePowerValue_10MIN,
                                         runtime, halttime, grGeneratorSpeed1Value_10MIN, grGridActivePowerValue_1HOUR]
            if None in argv_dict['input_vector'] or grgeneratorwindingtemperature4 is None or int(
                    grRunMode) != 14:  # 空或非并网的时候不运算
                pass
            else:
                ANN = BP(argv_dict)  # 采用神经网络
        ...
        if len(predict_value_list) > 0:
            ...
            healthy_score = query_last_state(wtgs_id, current_time, 'grGeneratorWindingTemperature4',
                                             'healthy_state_index')
        return healthy_score
    ...
    if queryRunMode(wtgs_id, current_time) != 14:
        ...
        for delta in range(-5, 5):
            ...
            argv_dict['input_vector'] = [grGearboxoilTempertureValue, grGridActivePowerValue, grOutdoorTemperatureValue,
                                         grAirDensityValue, grGridActivePowerValue_30MIN, grGridActivePowerValue_10MIN,
                                         runtime, halttime, grGeneratorSpeed1Value_10MIN, grGridActivePowerValue_1HOUR]
            if None in argv_dict['input_vector'] or grgeneratorwindingtemperature5 is None or int(
                    grRunMode) != 14:  # 空或非并网的时候不运算
                pass
            else:
                ANN = BP(argv_dict)  # 采用神经网络
        ...
        if len(predict_value_list) > 0:
            ...
            healthy_score = query_last_state(wtgs_id, current_time, 'grGeneratorWindingTemperature5',
                                             'healthy_state_index')
        return healthy_score
...
def grGeneratorWindingTemperature6(attribute, wtgs_id, current_time, his):  # 发电机绕组6温度
    ...
    if queryRunMode(wtgs_id, current_time) != 14:
        ...
        if len(predict_value_list) > 0:
            ...
            healthy_score = query_last_state(wtgs_id, current_time, 'grGeneratorWindingTemperature6',
                                             'healthy_state_index')
        return healthy_score
...
def grGeneratorDEBearingTemperature(attribute, wtgs_id, current_time, his):  # 发电机齿轮箱侧轴承估计温度
    ...
    if queryRunMode(wtgs_id, current_time) != 14:
        ...
        for delta in range(-5, 5):
            ...
            grGeneratorGearboxsideBearingTemperatureValue_1HOURAgo = getFloatInterpoValuesFromGolden2(
                'grgeneratorgearboxsidebearingtemperature', wtgs_id, hisOrFurTime(base_time_loop, -5, -1, 0),
                hisOrFurTime(base_time_loop, 5, -1, 0), his)
            # 输入变量-有功功率：grgridactivepower
            grGridActivePowerValue = getFloatInterpoValuesFromGolden2('grgridactivepower', wtgs_id,
                                                                      hisOrFurTime(base_time_loop, -5, 0, 0),
                                                                      hisOrFurTime(base_time_loop, 0, 0, 0), his)
            ...
            grGeneratorGearboxsideBearingTemperature = getFloatInterpoValuesFromGolden2(
                'grgeneratorgearboxsidebearingtemperature', wtgs_id, hisOrFurTime(base_time_loop, -5, 0, 0),
                hisOrFurTime(base_time_loop, 0, 0, 0), his)
            actual_value_list.append(grGeneratorGearboxsideBearingTemperature)
            # 神经网络输入变量
            argv_dict['input_vector'] = [grGearboxoilTempertureValue,
                                         grGeneratorGearboxsideBearingTemperatureValue_1HOURAgo,
                                         grGridActivePowerValue, grOutdoorTemperatureValue,
                                         grGridActivePowerValue_30MIN, grGridActivePowerValue_10MIN,
                                         runtime, halttime, grGeneratorSpeed1Value_10MIN, grGridActivePowerValue_1HOUR]
            if None in argv_dict['input_vector'] or grGeneratorGearboxsideBearingTemperature is None or int(
                    grRunMode) != 14:  # 空或非并网的时候不运算
                pass
            else:
                ANN = BP(argv_dict)  # 采用神经网络
        ...
        if len(predict_value_list) > 0:
            ...
            healthy_score = query_last_state(wtgs_id, current_time, 'grGeneratorDEBearingTemperature',
                                             'healthy_state_index')
        return healthy_score
...
def grGeneratorNDEBearingTemperature(attribute, wtgs_id, current_time, his):  # 发电机机舱侧轴承估计温度
    ...
    if queryRunMode(wtgs_id, current_time) != 14:
        ...
        for delta in range(-5, 5):
            ...
            # 输入变量-1小时前的发电机前轴承温度：grgeneratorgearboxsidebearingtemperature
            grGeneratorGearboxsideBearingTemperatureValue_1HOURAgo = getFloatInterpoValuesFromGolden2(
                'grgeneratornacellesidebearingtemperature', wtgs_id, hisOrFurTime(base_time_loop, -5, -1, 0),
                hisOrFurTime(base_time_loop, 5, -1, 0), his)
            # 输入变量-有功功率：grgridactivepower
            grGridActivePowerValue = getFloatInterpoValuesFromGolden2('grgridactivepower', wtgs_id,
                                                                      hisOrFurTime(base_time_loop, -5, 0, 0),
            ...
            grGeneratorNacellesideBearingTemperature = getFloatInterpoValuesFromGolden2(
                'grgeneratornacellesidebearingtemperature', wtgs_id, hisOrFurTime(base_time_loop, -5, 0, 0),
                hisOrFurTime(base_time_loop, 0, 0, 0), his)
            ...
            argv_dict['input_vector'] = [grGearboxoilTempertureValue,
                                         grGeneratorGearboxsideBearingTemperatureValue_1HOURAgo,
                                         grGridActivePowerValue, grOutdoorTemperatureValue,
                                         grGridActivePowerValue_30MIN,
                                         grGridActivePowerValue_10MIN, runtime, halttime, grGeneratorSpeed1Value_10MIN,
                                         grGridActivePowerValue_1HOUR]
            if None in argv_dict['input_vector'] or grGeneratorNacellesideBearingTemperature is None or int(
                    grRunMode) != 14:  # 空或非并网的时候不运算
                pass
            else:
                ANN = BP(argv_dict)  # 采用神经网络
        ...
        if len(predict_value_list) > 0:
            ...
            healthy_score = query_last_state(wtgs_id, current_time, 'grGeneratorNDEBearingTemperature',
                                             'healthy_state_index')
        return healthy_score
...
def grGearboxMainPumpMotorOperationTime1(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time,
                                         his):  # 齿轮箱主泵1运行时间
    # TODO-TESTED
    res = descend(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time, his)
    if res is None:  # 如果返回值为空
        value_last_state = query_last_state(wtgs_id, current_time, 'grGearboxMainPumpMotorOperationTime1',
                                            'healthy_state_index')
        if value_last_state == 0:
            return 1
        else:
...
def grGearboxMainPumpMotorOperationTime2(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time,
                                         his):  # 齿轮箱主泵2运行时间
    # TODO-TESTED
    res = descend(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time, his)
    if res is None:  # 如果返回值为空
        value_last_state = query_last_state(wtgs_id, current_time, 'grGearboxMainPumpMotorOperationTime2',
                                            'healthy_state_index')
        if value_last_state == 0:
            return 1
        else:
...
def grGearboxOilPressureA2(attribute, wtgs_id, current_time, his):  # 齿轮箱A2压力
    ...
    if queryRunMode(wtgs_id, current_time) != 14:
        ...
        for delta in range(-5, 5):
            ...
            gbGearboxPumpHighSpeedon1 = getIntInterpoValuesFromGolden3('gbgearboxpumphighspeedon1', wtgs_id,
                                                                       hisOrFurTime(base_time_loop, -5, 0, 0),
                                                                       hisOrFurTime(base_time_loop, 0, 0, 0), his)
            # 输入变量-齿轮箱油泵2高速：gbgearboxpumphighspeedon2
            gbGearboxPumpHighSpeedon2 = getIntInterpoValuesFromGolden3('gbgearboxpumphighspeedon2', wtgs_id,
                                                                       hisOrFurTime(base_time_loop, -5, 0, 0),
                                                                       hisOrFurTime(base_time_loop, 0, 0, 0), his)
            # 输入变量-齿轮箱油泵1低速：gbgearboxpumplowerspeedon1
            gbGearboxPumpLowerSpeedOn1 = getIntInterpoValuesFromGolden3('gbgearboxpumplowerspeedon1', wtgs_id,
                                                                        hisOrFurTime(base_time_loop, -5, 0, 0),
                                                                        hisOrFurTime(base_time_loop, 0, 0, 0), his)
            # 输入变量-齿轮箱油泵2低速：gbgearboxpumphighspeedon1
            gbGearboxPumpLowerSpeedOn2 = getIntInterpoValuesFromGolden3('gbgearboxpumplowerspeedon2', wtgs_id,
                                                                        hisOrFurTime(base_time_loop, -5, 0, 0),
                                                                        hisOrFurTime(base_time_loop, 0, 0, 0), his)
            # 实际输出变量-齿轮箱A2压力：grgearboxoilpressurea2
            grGearboxOilPressureA2 = getFloatInterpoValuesFromGolden2('grgearboxoilpressurea2', wtgs_id,
                                                                      hisOrFurTime(base_time_loop, -5, 0, 0),
                                                                      hisOrFurTime(base_time_loop, 0, 0, 0), his)
            ...
            argv_dict['input_vector'] = [grGearboxoilTempertureValue, grgearboxhubsidebearingtemperature,
                                         gbGearboxPumpHighSpeedon1, gbGearboxPumpHighSpeedon2,
                                         gbGearboxPumpLowerSpeedOn1, gbGearboxPumpLowerSpeedOn2]
            if None in argv_dict['input_vector'] or grGearboxOilPressureA2 is None or int(
                    grRunMode) != 14:  # 空或非并网的时候不运算
                pass
...
def grGearboxOilPressureA3(attribute, wtgs_id, current_time, his):  # 齿轮箱A3压力
    ...
    if queryRunMode(wtgs_id, current_time) != 14:
        ...
        for delta in range(-5, 5):
            ...
            grGearboxOilPressureA3 = getFloatInterpoValuesFromGolden2('grgearboxoilpressurea3', wtgs_id,
                                                                      hisOrFurTime(base_time_loop, -5, 0, 0),
                                                                      hisOrFurTime(base_time_loop, 0, 0, 0), his)
            ...
            argv_dict['input_vector'] = [grGearboxoilTempertureValue, grgearboxhubsidebearingtemperature,
                                         gbGearboxPumpHighSpeedon1, gbGearboxPumpHighSpeedon2,
                                         gbGearboxPumpLowerSpeedOn1, gbGearboxPumpLowerSpeedOn2]
            if None in argv_dict['input_vector'] or grGearboxOilPressureA3 is None or int(
                    grRunMode) != 14:  # 空或非并网的时候不运算
                pass
            elif gbGearboxPumpHighSpeedon1 == 1 and gbGearboxPumpHighSpeedon2 == 1:
                ANN = BP(argv_dict)  # 采用神经网络
...
def grGearboxOilPressureA4(attribute, wtgs_id, current_time, his):  # 齿轮箱A4压力
    ...
    if queryRunMode(wtgs_id, current_time) != 14:
        ...
        for delta in range(-5, 5):
            ...
            grGearboxOilPressureA4 = getFloatInterpoValuesFromGolden2('grgearboxoilpressurea4', wtgs_id,
                                                                      hisOrFurTime(base_time_loop, -5, 0, 0),
                                                                      hisOrFurTime(base_time_loop, 0, 0, 0), his)
            ...
            argv_dict['input_vector'] = [grGearboxoilTempertureValue, grgearboxhubsidebearingtemperature,
                                         gbGearboxPumpHighSpeedon1, gbGearboxPumpHighSpeedon2,
                                         gbGearboxPumpLowerSpeedOn1, gbGearboxPumpLowerSpeedOn2]
            if None in argv_dict['input_vector'] or grGearboxOilPressureA4 is None or int(
                    grRunMode) != 14:  # 空或非并网的时候不运算
                pass
            elif gbGearboxPumpHighSpeedon1 == 1 and gbGearboxPumpHighSpeedon2 == 1:
                ANN = BP(argv_dict)  # 采用神经网络
...
def grGearboxLubricationPumpHealthyLevel(wtgs_id, current_time, para_value_dict):  # 齿轮箱润滑泵健康度
    ...
    if flag == 0:
        ...
        value_last_state = query_last_state(wtgs_id, current_time, 'grGearboxLubricationPumpHealthyLevel',
                                            'healthy_state_index')
        if value_last_state == 0:
            return 1
        else:
...
def grGearboxMainPumpFilter1ErrorTemperature(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time,
                                             his):  # 齿轮箱主泵滤芯1堵塞时温度
...
def grGearboxMainPumpFilter2ErrorTemperature(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time,
                                             his):  # 齿轮箱主泵滤芯2堵塞时温度
...
def grGearboxMainPumpFilterHealthyLevel(wtgs_id, current_time, para_value_dict):  # 齿轮箱主泵滤芯健康度
    ...
    if flag == 0:
        ...
        value_last_state = query_last_state(wtgs_id, current_time, 'grGearboxMainPumpFilterHealthyLevel',
                                            'healthy_state_index')
        if value_last_state == 0:
            return 1
        else:
...
def grGearboxOilCoolerMotorOperationTime(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time,
                                         his):  # 齿轮箱油冷却器电机运行时间
    # TODO-TESTED
    res = descend(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time, his)
    if res is None:  # 如果返回值为空
        value_last_state = query_last_state(wtgs_id, current_time, 'grGearboxOilCoolerMotorOperationTime',
                                            'healthy_state_index')
        if value_last_state == 0:
            return 1
        else:
...
def grGearboxOilTemperature(attribute, wtgs_id, current_time, his):  # 齿轮箱油温
    ...
    if queryRunMode(wtgs_id, current_time) != 14:
        ...
        for delta in range(-5, 5):
            ...
            gbGearboxOilFanonValue = getIntInterpoValuesFromGolden3('gbgearboxoilfanon', wtgs_id,
                                                                    hisOrFurTime(base_time_loop, -1, 0, 0),
                                                                    hisOrFurTime(base_time_loop, 0, 0, 0), his)
            # 输入变量-齿轮箱主轴承温度：grgearboxmainbearingtemperature
            grGearboxMainBearingTemperatureValue = getFloatInterpoValuesFromGolden2('grgearboxmainbearingtemperature',
                                                                                    wtgs_id,
                                                                                    hisOrFurTime(base_time_loop, -5, 0,
                                                                                                 0),
                                                                                    hisOrFurTime(base_time_loop, 0, 0,
            ...
            grGearboxGeneratorSideBearingTemperatureValue = getFloatInterpoValuesFromGolden2(
                'grgearboxgeneratorsidebearingtemperature', wtgs_id, hisOrFurTime(base_time_loop, -5, 0, 0),
                hisOrFurTime(base_time_loop, 0, 0, 0), his)
            # 输入变量-齿轮箱前轴承温度：grgearboxhubsidebearingtemperature
            grGearboxHubsideBearingTemperatureValue = getFloatInterpoValuesFromGolden2(
                'grgearboxhubsidebearingtemperature', wtgs_id, hisOrFurTime(base_time_loop, -5, 0, 0),
                hisOrFurTime(base_time_loop, 0, 0, 0), his)
            # 输入变量-30分钟平均功率：grgridactivepower
            grGridActivePowerValue_30MIN = getFloatInterpoValuesFromGolden2('grgridactivepower', wtgs_id,
                                                                            hisOrFurTime(base_time_loop, -1800, 0, 0),
            ...
            gbGearboxOilFanOn_1hour = gbGearboxOilFanOn('gbgearboxoilfanon', wtgs_id,
                                                        hisOrFurTime(base_time_loop, -3600, 0, 0),
                                                        hisOrFurTime(base_time_loop, 0, 0, 0), his)
            # 输入变量-本次机组运行时间：runtime
            # 输入变量-本次机组停机时间：halttime
            ...
            grGearboxOilTemperture = getFloatInterpoValuesFromGolden2('grgearboxoiltemperture', wtgs_id,
                                                                      hisOrFurTime(base_time_loop, -5, 0, 0),
                                                                      hisOrFurTime(base_time_loop, 0, 0, 0), his)
            actual_value_list.append(grGearboxOilTemperture)
            # 神经网络输入变量
            argv_dict['input_vector'] = [grOutdoorTemperatureValue, gbGearboxOilFanonValue,
                                         grGearboxMainBearingTemperatureValue,
                                         grGearboxGeneratorSideBearingTemperatureValue,
                                         grGearboxHubsideBearingTemperatureValue,
                                         grGridActivePowerValue_30MIN, grGridActivePowerValue_10MIN,
                                         gbGearboxOilFanOn_1hour, runtime,
                                         halttime, grGeneratorSpeed1Value_10MIN, grWindSpeedValue_10MIN,
                                         grGearboxOilLevel_10MIN]
            if None in argv_dict['input_vector'] or grGearboxOilTemperture is None or int(
                    grRunMode) != 14:  # 空或非并网的时候不运算
                pass
...
def grGearboxBypassPumpMotorOperationTime(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time,
                                          his):  # 齿轮箱过滤泵电机运行时间
    # TODO-TESTED
    res = descend(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time, his)
    if res is None:  # 如果返回值为空
        value_last_state = query_last_state(wtgs_id, current_time, 'grGearboxBypassPumpMotorOperationTime',
                                            'healthy_state_index')
        if value_last_state == 0:
            return 1
        else:
...
def grGearboxBypassPumpFilterErrorTemperature(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time,
                                              his):  # 齿轮箱过滤泵滤芯堵塞温度
...
def grGearboxRadiatorHealthyLevel(wtgs_id, current_time, para_value_dict):  # 齿轮箱散热器健康度
    ...
    if flag == 0:
        ...
        value_last_state = query_last_state(wtgs_id, current_time, 'grGearboxRadiatorHealthyLevel',
                                            'healthy_state_index')
        if value_last_state == 0:
            return 1
        else:
...
def grGearboxBypassPartHealthyLevel(wtgs_id, current_time, para_value_dict):  # 齿轮箱过滤系统健康度
    ...
    if flag == 0:
        ...
        value_last_state = query_last_state(wtgs_id, current_time, 'grGearboxBypassPartHealthyLevel',
                                            'healthy_state_index')
        if value_last_state == 0:
            return 1
        else:
...
def grGearboxLubricatingPartHealthyLevel(wtgs_id, current_time, para_value_dict):  # 齿轮箱润滑系统润滑部分健康度
    ...
    for para, attr in para_value_dict.items():
        if attr['value'] is None:
            ...
            break
    if flag == 0:
        Motor = min(para_value_dict['grGearboxMainPumpMotorOperationTime1']['value'],
                    para_value_dict['grGearboxMainPumpMotorOperationTime2']['value'])
        Pump = numpy.mean(
            [para_value_dict['grGearboxOilPressureA2']['value'], para_value_dict['grGearboxOilPressureA3']['value'],
             para_value_dict['grGearboxOilpressureA4']['value']])
        ...
        return round(min(Motor, Pump, Filter, para_value_dict['grGearboxOilLevel']['value'],
                         para_value_dict['grGearboxOilCoolerMotorOperationTime']['value'],
                         para_value_dict['grGearboxOilTemperature']['value']), 4)
    else:
        loger.warning("齿轮箱润滑系统润滑部分健康度 " + str(wtgs_id) + " " + str(current_time) + " is empty!")
        value_last_state = query_last_state(wtgs_id, current_time, 'grGearboxLubricatingPartHealthyLevel',
                                            'healthy_state_index')
        if value_last_state == 0:
            return 1
        else:
...
def grGeneratorWaterPumpOnTime(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time,
                               his):  # 发电机水冷泵电机运行时间
    # TODO-TESTED
    res = descend(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time, his)
    if res is None:  # 如果返回值为空
...
def grGeneratorWaterPumpPressureDifference(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time,
                                           his):  # 发电机水冷泵进出口压力偏差
    ...
    if res == 0 or res is None:  # 如果返回值为空
        value_last_state = query_last_state(wtgs_id, current_time, 'grGeneratorWaterPumpPressureDifference',
                                            'healthy_state_index')
        if value_last_state == 0:
            return 1
        else:
...
def grGeneratorWaterFanOnTime(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time_now,
                              his):  # 发电机水冷风扇电机运行时间
    # TODO-TESTED
...
def grGeneratorWaterTemperature(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time,
                                his):  # 发电机冷却水估计温度
    # TODO-电机冷却水估计温度-无模型
    return 1
...
def grHydraulicPumpMotorOperationTime(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time,
                                      his):  # 液压泵电机运行时间
    # TODO-TESTED
    res = descend(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time, his)
    if res is None:  # 如果返回值为空
        value_last_state = query_last_state(wtgs_id, current_time, 'grHydraulicPumpMotorOperationTime',
                                            'healthy_state_index')
        if value_last_state == 0:
            return 1
        else:
...
def grHydraulicPumpEstablishPressureTime(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time,
                                         his):  # 液压泵建压时间
    # TODO-TESTED
    res = descend(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time, his)
    if res is None:  # 如果返回值为空
        value_last_state = query_last_state(wtgs_id, current_time, 'grHydraulicPumpEstablishPressureTime',
                                            'healthy_state_index')
        if value_last_state == 0:
            return 1
        else:
...
def giYawBrakeOpenTotalValveActiveTimes(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time,
                                        his):  # 偏航全释放阀动作次数
    # TODO-TESTED
    res = descend(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time, his)
    if res is None:  # 如果返回值为空
        value_last_state = query_last_state(wtgs_id, current_time, 'giYawBrakeOpenTotalValveActiveTimes',
                                            'healthy_state_index')
        if value_last_state == 0:
            return 1
        else:
...
def giYawBrakeOpenHalfValveActiveTimes(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time,
                                       his):  # 偏航半释放阀动作次数
    # TODO-TESTED
    res = descend(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time, his)
    if res is None:  # 如果返回值为空
        value_last_state = query_last_state(wtgs_id, current_time, 'giYawBrakeOpenHalfValveActiveTimes',
                                            'healthy_state_index')
        if value_last_state == 0:
            return 1
        else:
...
def giRotorLockValveActiveTimes(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time,
                                his):  # 叶轮锁阀动作次数
    # TODO-TESTED
    res = descend(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time, his)
    if res is None:  # 如果返回值为空
...
def giRotorBrakeValveActiveTimes(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time,
                                 his):  # 叶轮制动阀动作次数
    # TODO-TESTED
    res = descend(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time, his)
    if res is None:  # 如果返回值为空
        value_last_state = query_last_state(wtgs_id, current_time, 'giRotorBrakeValveActiveTimes',
                                            'healthy_state_index')
        if value_last_state == 0:
            return 1
        else:
...
def grNacelleFanOperationTime(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time,
                              his):  # 机舱风扇运行时间
    # TODO-TESTED
    res = descend(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time, his)
    if res is None:  # 如果返回值为空
...
def grYawBrakeWearAwayOperationTime(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time,
                                    his):  # 偏航制动器摩擦片运行时间
    # TODO-TESTED
    res = descend(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time, his)
    if res is None:  # 如果返回值为空
        value_last_state = query_last_state(wtgs_id, current_time, 'grYawBrakeWearAwayOperationTime',
                                            'healthy_state_index')
        if value_last_state == 0:
            return 1
        else:
...
def grConverterWaterPumpMotorOperationTime(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time,
                                           his):  # 变频器水冷泵电机运行时间
    # TODO-TESTED
    res = descend(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time, his)
    if res is None:  # 如果返回值为空
        value_last_state = query_last_state(wtgs_id, current_time, 'grConverterWaterPumpMotorOperationTime',
                                            'healthy_state_index')
        if value_last_state == 0:
            return 1
        else:
...
def grConverterWaterPumpInOutPressureDifference(index, fromValue, toValue, wtgs_id, from_time_now, current_time,
                                                to_time, his):  # 变频器水泵进出水压差
    # TODO-变频器水泵进出水压差-无标签点
    return 1


def grConverterWaterCoolingFanMotor1OperationTime(index, fromValue, toValue, wtgs_id, from_time_now, current_time,
                                                  to_time, his):  # 变频器水冷风扇电机1运行时间
    # TODO-TESTED
    res = descend(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time, his)
    if res is None:  # 如果返回值为空
        value_last_state = query_last_state(wtgs_id, current_time, 'grConverterWaterCoolingFanMotor1OperationTime',
                                            'healthy_state_index')
        if value_last_state == 0:
            return 1
        else:
...
def grConverterWaterCoolingFanMotor2OperationTime(index, fromValue, toValue, wtgs_id, from_time_now, current_time,
                                                  to_time, his):  # 变频器水冷风扇电机2运行时间
    # TODO-TESTED
    res = descend(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time, his)
    if res is None:  # 如果返回值为空
        value_last_state = query_last_state(wtgs_id, current_time, 'grConverterWaterCoolingFanMotor2OperationTime',
                                            'healthy_state_index')
        if value_last_state == 0:
            return 1
        else:
...
def grConverterWaterCoolingFanMotor3OperationTime(index, fromValue, toValue, wtgs_id, from_time_now, current_time,
                                                  to_time, his):  # 变频器水冷风扇电机3运行时间
    # TODO-TESTED
    res = descend(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time, his)
    if res is None:  # 如果返回值为空
        value_last_state = query_last_state(wtgs_id, current_time, 'grConverterWaterCoolingFanMotor3OperationTime',
                                            'healthy_state_index')
        if value_last_state == 0:
            return 1
        else:
...
def grConverterWaterCoolingFanMotorHealthyLevel(wtgs_id, current_time, para_value_dict):  # 变频器水冷风扇电机总健康度
    ...
    if flag == 0:
        ...
        value_last_state = query_last_state(wtgs_id, current_time, 'grConverterWaterCoolingFanMotorHealthyLevel',
                                            'healthy_state_index')
        if value_last_state == 0:
            return 1
        else:
...
def grConverterWaterTemperature(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time,
                                his):  # 变频器估计水温
    # TODO-变频器估计水温-无标签点
    return 1


def grConverterIGBTTemperature(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time,
                               his):  # 变频器IGBT估计水温
...
def grConverterCabinetFan1OperationTime(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time,
                                        his):  # 变频器控制柜风扇1运行时间
    # TODO-TESTED
    res = descend(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time, his)
    if res is None:  # 如果返回值为空
        value_last_state = query_last_state(wtgs_id, current_time, 'grConverterCabinetFan1OperationTime',
                                            'healthy_state_index')
        if value_last_state == 0:
            return 1
        else:
...
def grConverterCabinetFan2OperationTime(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time,
                                        his):  # 变频器控制柜风扇2运行时间
    # TODO-TESTED
    res = descend(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time, his)
    if res is None:  # 如果返回值为空
        value_last_state = query_last_state(wtgs_id, current_time, 'grConverterCabinetFan2OperationTime',
                                            'healthy_state_index')
        if value_last_state == 0:
            return 1
        else:
...
def grConverterCabinetFan3OperationTime(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time,
                                        his):  # 变频器控制柜风扇3运行时间
    # TODO-TESTED
    res = descend(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time, his)
    if res is None:  # 如果返回值为空
        value_last_state = query_last_state(wtgs_id, current_time, 'grConverterCabinetFan3OperationTime',
                                            'healthy_state_index')
        if value_last_state == 0:
            return 1
        else:
...
def giConverterLineSideMCBActionTimes(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time,
                                      his):  # 变频器网侧断路器动作次数
    # TODO-TESTED
    res = descend(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time, his)
    if res is None:  # 如果返回值为空
        value_last_state = query_last_state(wtgs_id, current_time, 'giConverterLineSideMCBActionTimes',
                                            'healthy_state_index')
        if value_last_state == 0:
            return 1
        else:
...
def grConverterGeneratorSideMCBActionTimes(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time,
                                           his):  # 变频器机侧断路器动作次数
    # TODO-TESTED
    res = descend(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time, his)
    if res is None:  # 如果返回值为空
        value_last_state = query_last_state(wtgs_id, current_time, 'grConverterGeneratorSideMCBActionTimes',
                                            'healthy_state_index')
        if value_last_state == 0:
            return 1
        else:
...
def giGearboxMainPumpOnTimes1(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time,
                              his):  # 齿轮箱主泵启动次数1
    # TODO-TESTED
    res = descend(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time, his)
    if res is None:  # 如果返回值为空
...
def giGearboxMainPumpOnTimes2(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time,
                              his):  # 齿轮箱主泵启动次数2
    # TODO-TESTED
    res = descend(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time, his)
    if res is None:  # 如果返回值为空
...
def giGearboxBypassPunpOnTimes(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time,
                               his):  # 齿轮箱旁通过滤泵启动次数
    # TODO-TESTED
    res = descend(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time, his)
    if res is None:  # 如果返回值为空
...
def giGearboxOilHeaterOnTimes(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time,
                              his):  # 齿轮箱油加热器启动次数
    # TODO-TESTED
    res = descend(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time, his)
    if res is None:  # 如果返回值为空
...
def giGearboxOilCoolerBypassValveTimes(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time,
                                       his):  # 齿轮箱油冷却滤油阀启动次数
    # TODO-TESTED
    res = descend(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time, his)
    if res is None:  # 如果返回值为空
        value_last_state = query_last_state(wtgs_id, current_time, 'giGearboxOilCoolerBypassValveTimes',
                                            'healthy_state_index')
        if value_last_state == 0:
            return 1
        else:
...
def giGearboxOilVolumeValveTimes(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time,
                                 his):  # 齿轮箱油量阀启动次数
    # TODO-TESTED
    res = descend(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time, his)
    if res is None:  # 如果返回值为空
        value_last_state = query_last_state(wtgs_id, current_time, 'giGearboxOilVolumeValveTimes',
                                            'healthy_state_index')
        if value_last_state == 0:
            return 1
        else:
...
def giGeneratorWaterPumpOnTimes(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time,
                                his):  # 发电机水冷泵启动次数
    # TODO-TESTED
    res = descend(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time, his)
    if res is None:  # 如果返回值为空
...
def giGeneratorWaterFanOnTimes(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time,
                               his):  # 发电机水冷风扇启动次数
    # TODO-TESTED
    res = descend(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time, his)
    if res is None:  # 如果返回值为空
...
def giGeneratorWaterHeaterOnTimes(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time,
                                  his):  # 发电机水加热器启动次数
    # TODO-TESTED
    res = descend(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time, his)
    if res is None:  # 如果返回值为空
        value_last_state = query_last_state(wtgs_id, current_time, 'giGeneratorWaterHeaterOnTimes',
                                            'healthy_state_index')
        if value_last_state == 0:
            return 1
        else:
...
def grNacelleCabinetUPSBatteryWorkTime(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time,
                                       his):  # 机舱柜UPS电池工作时间
...
def grTowerCabinetUPSBatteryWorkTime(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time,
                                     his):  # 塔基柜UPS电池工作时间
    # TODO-TESTED-存储值为0或空时强制设为1
    res = ascend(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time, his)
    if res == 0 or res is None:
...
def ascend1(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time_now,
            his):  # 线性增加型-空载电压适用：空载电压为0时默认健康度为1
    if fromValue > toValue:
        fromValue, toValue = toValue, fromValue
    tag_golden_index = readTagIndex(index, wtgs_id)  # 查询标签点在golden中的id
    try:
...
def ascendMultiVariable(index, fromValue, toValue, wtgs_id, from_time_now, current_time, to_time_now, his):  # 多标签点线性增加型
    ...
    value_dict = value_dict[(value_dict['giwindturbineoperationmode'] == 14.0) & (
        value_dict['gbgeneratorwaterpumpon'] == 1.0)]  # 机组并网且发电机水泵启动信号
    degreeList = []
    if len(value_dict) > 0:
        for row in range(len(value_dict)):
            diff = abs(
                value_dict['grgeneratorwaterpressurein'].iloc[row] - value_dict['grgeneratorwaterpressureout'].iloc[
                    row])  # 两者差值的绝对值
            if diff > toValue:
                degreeList.append(1)
...
def readTagIndex(tag_EN, wtgs_id):  # 查询标签点在庚顿数据库中的索引
    ...
    sqlstr = "SELECT tag_index_golden FROM tag_golden_index WHERE wtgs_id=\'" + str(wtgs_id) + "\' AND tag_EN=\'" + str(
        tag_EN) + "\'"
...
def getFloatInterpoValuesFromGolden(tag_id, start_time, end_time, his):  # 查浮点型插值数据
    ...
    result = his.getFloatInterpoValues(tag_id, count, data_unit.stringToDate(start_time),
                                       data_unit.stringToDate(end_time))
    if result.size() > 0:
        values = []
...
def getIntInterpoValuesFromGolden(tag_id, start_time, end_time, his):  # 查整型插值数据
    data_unit = autoclass('com.rtdb.api.util.DateUtil')
    count = pd.date_range(start=start_time, end=end_time, freq='S').size
    result = his.getIntInterpoValues(tag_id, count, data_unit.stringToDate(start_time),
                                     data_unit.stringToDate(end_time))
    if result.size() > 0:
        values = []
        for i in range(result.size()):
...
def gbGearboxOilFanOn(tag_EN, wtgs_id, start_time, end_time, his):  # 查数据
    ...
    data_unit = autoclass('com.rtdb.api.util.DateUtil')
    count = pd.date_range(start=start_time, end=end_time, freq='S').size
    result = his.getIntInterpoValues(tag_golden_index, count, data_unit.stringToDate(start_time),
                                     data_unit.stringToDate(end_time))
    if result.size() > 1:
        values = []
        turn_on_times = 0
...
class BP:
    def __init__(self, argv_dict):
        ...
        self.bias = argv_dict['bias']  # 偏置量字典, 各层的维数不一致
    ...
    def networks(self):  # 网络运算, 获得输出
        ...
        for layer in range(len(self.weight) - 1):  # 隐含层
            ...
            local_layer_out = numpy.mat(
                [float(numpy.exp(x[0]) - numpy.exp(-x[0])) / float(numpy.exp(x[0]) + numpy.exp(-x[0])) for x in
                 local_layer_out.tolist()]).T  # sigmoid激活函数
...
def query_last_state(wtgs_id, current_time, tag, table):
    ...
    cur = conn.cursor()
    sqlstr = "SELECT " + tag + " FROM " + table + " WHERE wtgsId=\'" + str(
        wtgs_id) + "\' AND realTime=\'" + one_hour_before + "\'"
    # #print(sqlstr)
    cur.execute(sqlstr)
...
def queryRunMode(wtgs_id, current_time):  # 查询最新时间current_time机组的运行模式
    conn = pymysql.connect(host='192.168.0.19', port=3306, user='llj', passwd='llj@2016', db='iot_wind', charset="utf8")
    sqlstr = "SELECT runMode FROM healthy_state_run_halt_time WHERE wtgsId=\'" + str(
        wtgs_id) + "\' AND realTime=(SELECT MAX(realTime) FROM healthy_state_run_halt_time WHERE wtgsId=\'" + str(
        wtgs_id) + "\' AND realTime<=\'" + current_time + "\')"
...
def queryRunHaltTime(wtgs_id, current_time):
    ...
    conn = pymysql.connect(host='192.168.0.19', port=3306, user='llj', passwd='llj@2016', db='iot_wind', charset="utf8")
    sqlstr = "SELECT * FROM healthy_state_run_halt_time WHERE wtgsId=\'" + str(
        wtgs_id) + "\' AND realTime=(SELECT MAX(realTime) FROM healthy_state_run_halt_time WHERE wtgsId=\'" + str(
        wtgs_id) + "\' AND realTime<=\'" + current_time + "\')"
    latest_cal_state = pd.read_sql(sql=sqlstr, con=conn)
    conn.close()
    ...
    if int(latest_cal_state['runMode'].iloc[0]) in [12, 13, 14]:
        run_time = float(latest_cal_state['runTime'].iloc[0]) + (end_time - start_time).seconds / 60 + (
                                                                                                           end_time - start_time).days * 24 * 60
        if run_time > 600:
            run_time = 600
        halt_time = float(latest_cal_state['haltTime'].iloc[0]) - (end_time - start_time).seconds / 60 - (
                                                                                                             end_time - start_time).days * 24 * 60
        if halt_time < 0:
            halt_time = 0
    else:
...
def hisOrFurTime(base_time, seconds_delta, hours_delta, days_delta):
    # 时间范围
    hisOrFurTime = datetime.strptime(base_time, "%Y-%m-%d %H:%M:%S") + timedelta(seconds=seconds_delta) + timedelta(
        hours=hours_delta) + timedelta(days=days_delta)
...
def weightBias(attribute):
    ...
    # 输出层
    weigth_str_list = [float(jtem) for jtem in
                       [item for item in attribute['w' + str(int(attribute['layers']))].split(',')]]
...
if __name__ == "__main__":
    engine = create_engine('sqlite:///config.db')  # 用sqlalchemy创建引擎
    #print(pd.read_sql('tag_golden_index', con=engine))
